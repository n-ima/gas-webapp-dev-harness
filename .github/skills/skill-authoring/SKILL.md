...
具体的なCI設定（lint/test/deployのworkflow）は事前に同梱していない。
**設計フェーズでCIを使うと決めた時点で、このスキルの手順に従いCIワークフローを生成する**
（設計文書がCIを前提にするなら、その実体もこのタイミングで作る。
「文書がCIを参照しているのに実体がない」状態を放置しない）。
生成した `.github/workflows/` はハーネス保護フックの対象になるため、
以後の変更は人間のレビューを経由する。

**GAS（このハーネスの固定スタック）の場合**: GitHub Actions + clasp による push 自動化が
定番。認証情報（`~/.clasprc.json` 相当）は **GitHub Secrets に保管**し、ワークフロー内で
書き出して使う。コード・リポジトリへの記載は絶対に不可。トークン失効時の更新運用も含めた
具体的な要点は `.github/skills/deploy-gas/SKILL.md` の「CI/CD」節を正とする。
