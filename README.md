# gas-webapp-dev-harness — GAS特化AI開発ハーネス（Copilot / Claude Code / Antigravity対応）

**Google Apps Script（GAS）のアプリ**（スプレッドシート連携ツール・Webアプリを含む）の
「要件定義 → 設計 → 実装 → テスト → リリース」を一気通貫でオーケストレーションするための
**GAS特化テンプレート** です。特定のアプリのコードは含みません。新しいGASアプリを作るたびに、
このリポジトリをテンプレートとして使い、要件メモを書くところから始めます。

汎用ハーネス [copilot-sdlc-harness](https://github.com/n-ima/copilot-sdlc-harness) からの
派生で、技術スタックがGASに固定されている利点を活かし、GASの規約
（`.github/skills/stack-conventions/`）・デプロイ手順（`.github/skills/deploy-gas/`）・
環境テンプレート（`docs/01-requirements/environment_template.md`）を裏取り済みの現行仕様
（clasp v3 等）で**事前確定**してあります（系譜と根拠は [DECISIONS.md](DECISIONS.md)）。
...
- **使う**: 本番運用・長期保守するGASアプリ（スプレッドシート連携・Webアプリ・トリガー運用）、
  複数人での開発、監査・トレーサビリティ要件のある開発。
- **使わない**: 使い捨てスクリプト、実験、PoC（素のCopilotに直接頼む方が速い）。
- **使わない**: **GAS以外の技術スタック**（Webサービス一般、モバイル、データ基盤等）。
  その場合は汎用の本体 [copilot-sdlc-harness](https://github.com/n-ima/copilot-sdlc-harness)
  を使う。
...
.github/
  ...
  skills/                       … フェーズで使う手順・チェックリスト（Agent Skills）
                                    GAS固有: stack-conventions（GAS規約）, deploy-gas（デプロイ手順）
                                    共通: security-review, skill-authoring 等
...
- **技術スタックはGASに固定**: 本体ハーネスの「技術スタック非依存」を意図的に捨て、
  GASの規約・デプロイ手順・制限値を事前確定することで、設計〜リリースの調査コストを
  ゼロにしている。設計フェーズのトレードオフ提示は「GAS内での選択」
  （コンテナバインド vs スタンドアロン、実行ユーザー、アクセス範囲、UI構成等）に絞られる。
- **エージェントを薄く、手順はSkillへ**: 各エージェントファイルは人格・権限・引き継ぎ先だけを持ち、
  詳細な手順・チェックリストはAgent Skillsに分離する。
- **環境固有Skillは事前同梱済み（本体との違い）**: デプロイ先がGASに固定のため、
  本体の「環境が判明した時点で動的作成」ではなく `deploy-gas` / `stack-conventions` を
  事前同梱している。GAS以外の外部サービス（例: 外部API連携先）が加わった場合のみ、
  `skill-authoring` スキルで追加のSkillをその場で育てる。
